            ...
            <div class="card-body">
                <form method="post" action="{{ route('permission.update', $permission->id) }}">
                    ...
                    <div class="row">
                        <div class="form-group col-md-6 mb-3">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                                id="permission_name" value="{{ old('name', $permission->name) }}" placeholder="Edit Permission">
                            @error('name')
                        ...
                        <div class="form-group col-md-6 mb-3">
                            ...
                            <select name="group_name" class="form-select @error('group_name') is-invalid @enderror" id="group_name">
                                ...
                                @foreach ($permissionGroups as $value => $label)
                                    <option value="{{ $value }}" @selected(old('group_name', $permission->group_name) === $value)>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('group_name')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary waves-effect waves-light">Update Permission</button>
